...
- Public npm package name `@pacharapolpimpa/reposetup` (single bundled CLI; executable `reposetup`). Unscoped `reposetup` and `reposetup-cli` are blocked by npm as too similar to `repo-setup` and `repo-setup-cli`. Workspace libraries `@reposetup/core`, `@reposetup/registry`, and `@reposetup/integrations` stay private.
...
- `@pacharapolpimpa/reposetup@0.1.0` is prepared for npm; publication waits on maintainer 2FA after `npm login`.
