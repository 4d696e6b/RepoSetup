...
| `publish-npm.yml` | Tag-only publish of `rsetup` via npm trusted publishing (OIDC) |
...
1. An npm user that will own unscoped `rsetup`
...
8. Trusted publishing sends `rsetup` to npm (after npmjs.com is configured)
9. Verify `npm view rsetup@version version`
