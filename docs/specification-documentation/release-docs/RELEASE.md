...
The public npm package is **`reposetup-cli`** (Model A: one bundled CLI; executable `reposetup`). Workspace libraries stay private. The root workspace is `@reposetup/workspace`. Unscoped `reposetup` is blocked by npm similarity to `repo-setup`.
...
4. Inspect `packages/cli/reposetup-cli-0.1.0.tgz` from `pnpm pack:packages`
...
| `publish-npm.yml` | Tag-only publish of `reposetup-cli` via npm trusted publishing (OIDC) |
...
1. An npm user that will own unscoped `reposetup-cli`
...
8. Trusted publishing sends `reposetup-cli` to npm (after npmjs.com is configured)
9. Verify `npm view reposetup-cli@version version`
