...
The public npm package is **`@pacharapolpimpa/reposetup`** (Model A: one bundled CLI; executable `reposetup`). Workspace libraries stay private. The root workspace is `@reposetup/workspace`. Unscoped `reposetup` and `reposetup-cli` are blocked by npm similarity to `repo-setup` and `repo-setup-cli`.
...
4. Inspect `packages/cli/pacharapolpimpa-reposetup-0.1.0.tgz` from `pnpm pack:packages`
...
| `publish-npm.yml` | Tag-only publish of `@pacharapolpimpa/reposetup` via npm trusted publishing (OIDC) |
...
1. An npm user that will own scoped `@pacharapolpimpa/reposetup`
...
8. Trusted publishing sends `@pacharapolpimpa/reposetup` to npm (after npmjs.com is configured)
9. Verify `npm view @pacharapolpimpa/reposetup@version version`
