...
Public GitHub launch plus Model A npm packaging (2026-09-22): public package name `reposetup-cli`; workspace libraries private and bundled into the CLI.
...
- No npm trusted-publisher config until `reposetup-cli` exists on the registry. First publish needs maintainer `npm login` + OTP from `packages/cli`.
- Unscoped `reposetup` is blocked by npm as too similar to `repo-setup`.
...
| Public package name | `reposetup-cli` (unscoped `reposetup` blocked by similarity to `repo-setup`) |
...
| Tarball | `reposetup-cli-0.1.0.tgz`; isolated install covered by `pnpm test:e2e` |
| Packed golden | React + Vite create + `stack` + `doctor` + `tsc -b` passed from an earlier `reposetup` tarball; re-verify after rename |
