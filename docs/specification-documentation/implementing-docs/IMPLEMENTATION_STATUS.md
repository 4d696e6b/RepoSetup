...
Public GitHub launch plus Model A npm packaging (2026-09-22): public package name `@pacharapolpimpa/reposetup`; workspace libraries private and bundled into the CLI.
...
- No npm trusted-publisher config until `@pacharapolpimpa/reposetup` exists on the registry. First publish needs maintainer `npm login` + OTP from `packages/cli`.
...
| Public package name | `@pacharapolpimpa/reposetup` (unscoped names blocked by similarity) |
...
| Tarball | `pacharapolpimpa-reposetup-0.1.0.tgz`; isolated install covered by `pnpm test:e2e` |
