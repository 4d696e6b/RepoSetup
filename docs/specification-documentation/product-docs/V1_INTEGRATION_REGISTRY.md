...
"Planned" and "P0" do not mean `stable`. RepoSetup `0.1.0` classifies each ID as `experimental`, `candidate`, `stable`, or `deprecated`. See `docs/specification-documentation/implementing-docs/INTEGRATION_SUPPORT.md` and `reposetup info <id>`. No catalog ID is `stable` in this release.
