...
| Field | Value |
| --- | --- |
| Public package name | `reposetup-cli` |
| First version | `0.1.0` |
...
The npm **username** is whatever account owns `reposetup-cli` after the first publish. Do not assume it equals the GitHub login.
...
## After `reposetup-cli` exists on npm

1. Open `https://www.npmjs.com/package/reposetup-cli`.
...
8. Confirm `npm view reposetup-cli@version version`.
