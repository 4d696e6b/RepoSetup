...
describe("npm pack artifact", () => {
  it("packs the public CLI, inspects the tarball, and runs the installed binary", async () => {
    ...
    const packed = await runProcess(
      "pnpm",
      ["--filter", "reposetup", "pack", "--pack-destination", packDir],
      {
        cwd: repoRoot,
      },
    );
    ...
    expect(tarballs).toEqual(["reposetup-0.1.0.tgz"]);
    ...
    expect(packedManifest.name).toBe("reposetup");
    ...
    const artifactBin = path.join(installDir, "node_modules", "reposetup", "dist", "bin.js");
